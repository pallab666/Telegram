...
import { Globe, DollarSign, Check, Sparkles, ArrowRight } from 'lucide-react';
import {
  AppLanguage,
  AppCurrency,
  AppPreferences,
...
export const InitialSetupModal: React.FC<InitialSetupModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/80 backdrop-blur-md animate-in fade-in">
      <div className="relative w-full max-w-sm bg-white border border-slate-200 rounded-3xl overflow-hidden shadow-2xl flex flex-col animate-in zoom-in-95 duration-200">
        {/* Top Header Banner */}
        <div className="p-5 bg-gradient-to-br from-indigo-950 via-slate-900 to-slate-950 text-white relative overflow-hidden">
          ...
          <p className="text-xs text-slate-300 mt-2 leading-relaxed">
            {isBn
              ? 'অ্যাপে প্রবেশ করার আগে আপনার পছন্দের ভাষা ও মুদ্রা বেছে নিন। পরবর্তীতে সেটিংস থেকে পরিবর্তন করতে পারবেন।'
              : 'Choose your preferred display language and currency. You can adjust this anytime in Settings.'}
          </p>
        </div>
        ...
        <div className="p-5 space-y-5 bg-slate-50 overflow-y-auto max-h-[68vh]">
              ...
              {(Object.keys(CURRENCY_CONFIGS) as AppCurrency[]).map((curKey) => {
                ...
                return (
                  <button
                    ...
                    onClick={() => {
                      triggerHaptic('light');
                      setSelectedCurrency(curKey);
                    }}
                    className={`p-2.5 rounded-xl border text-left transition-all flex items-center justify-between cursor-pointer ${
                      isSelected
                        ? 'border-emerald-600 bg-emerald-50/90 text-emerald-950 font-bold ring-2 ring-emerald-500/20 shadow-sm'
                        : 'border-slate-200 bg-white text-slate-700 hover:bg-slate-100/70'
                    }`}
                  >
          ...
          <button
            ...
            className="w-full py-3.5 px-4 rounded-2xl bg-gradient-to-r from-indigo-600 via-indigo-700 to-purple-600 hover:from-indigo-700 hover:to-purple-700 text-white font-bold text-sm flex items-center justify-center gap-2 shadow-lg shadow-indigo-600/25 active:scale-98 transition-all cursor-pointer"
          >
